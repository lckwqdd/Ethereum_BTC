...
import com.alsc.net.api.WTransferApi;
 ...
 * 以太坊转账
 */
public class HotETHTransferActivity extends AlscBaseActivity implements View.OnClickListener {
    private static final String TAG = "HotETHTransferActivity";
    ...
    private void getNonceFromSendAddress(String address) {
        ThreadUtils.executeBySingleAtFixRate(new ThreadUtils.SimpleTask<String>() {
            ...
            public String doInBackground() throws Throwable {
                String tempNonce = UrlRequstUtils.getNonceFromSendAddress(HotETHTransferActivity.this, address);
                return tempNonce;
            }
...
//                  LogUtils.d("当前钱包:"+ GsonUtils.toJson(ethWalletList.get(0)));
                    ...
                    if (!TextUtils.isEmpty(hexValue)) {
                        ethTranstionBean ethTtrantion = UrlRequstUtils.sendEthBrocastTranstion(HotETHTransferActivity.this, hexValue);
            ...
            Observable.create(new ObservableOnSubscribe<ethTranstionBean>() {
                ...
                public void subscribe(ObservableEmitter<ethTranstionBean> e) throws Exception {
                    ...
                    if (!TextUtils.isEmpty(txHash)) {
                        //发送交易广播
                        ethTranstionBean ethTtrantion = UrlRequstUtils.sendEthBrocastTranstion(HotETHTransferActivity.this, txHash);
                        e.onNext(ethTtrantion);
                    }
                }
            }).subscribeOn(Schedulers.io())
    ...
    private void hotTransfer(float price, float amount, String fromAddress, String toAddress) {
        ...
        request.setSend_url(fromAddress);
        ...
        }), HotETHTransferActivity.this, request));
    }
