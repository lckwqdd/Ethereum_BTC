...
public class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!AppUtil.isNetworkAvailable(NetApplication.getInstance().getContext())) {//没网强制从缓存读取(必须得写，不然断网状态下，退出应用，或者等待一分钟后，就获取不到缓存）
        ...
        if (AppUtil.isNetworkAvailable(NetApplication.getInstance().getContext())) {
