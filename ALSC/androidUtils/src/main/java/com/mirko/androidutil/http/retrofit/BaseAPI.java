...
import com.mirko.androidutil.http.exception.ApiException;
import com.mirko.androidutil.http.retrofit.subscriber.ProgressSubscriber;
...
public abstract class BaseAPI {
    ...
    public static <T> ObservableTransformer<BaseResponse<T>, T> handleResult() {
        return new ObservableTransformer<BaseResponse<T>, T>() {
            @Override
            public Observable<T> apply(Observable<BaseResponse<T>> tObservable) {
                return tObservable.flatMap(new Function<BaseResponse<T>, ObservableSource<T>>() {
                    @Override
                    public Observable<T> apply(BaseResponse<T> result) {
                        //成功后交给界面处理
                        if (result.getStatusCode() == HttpConfig.successCode) {
                            return createData(result.getData());
                        } else {
                            //统一处理服务器返回值非正常结果
                            Log.d(HttpConfig.TAG, "BaseRequestBusiness统一处理服务器返回值非正常结果apply: " + HttpConfig.getApiReason(result.getStatusCode()));
                            return Observable.error(new ApiException(HttpConfig.getApiReason(result.getStatusCode())));
                        }
                    }
                })
